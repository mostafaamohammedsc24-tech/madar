...
class ClosingLoginScreen extends StatefulWidget {
  const ClosingLoginScreen({super.key});

  @override
  State<ClosingLoginScreen> createState() => _ClosingLoginScreenState();
}

class _ClosingLoginScreenState extends State<ClosingLoginScreen> {
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Text(loc.notPublic, style: LegalTheme.ibm(size: 12, color: LegalTheme.muted)),
            ...
            if (auth.message != null) ...[
              const SizedBox(height: 12),
              AuthErrorBanner(message: loc.invalid),
            ],
  ...
  Future<void> _submit(ClosingAuthNotifier auth) async {
    final ok = await auth.login(employeeId: _id.text, secret: _secret.text);
    if (!mounted) return;
    if (ok) context.go('/closing/work');
  }
}
