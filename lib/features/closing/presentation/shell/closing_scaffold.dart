...
class ClosingScaffold extends StatelessWidget {
  const ClosingScaffold({required this.navigationShell, super.key});
...
class ClosingTopBar extends StatelessWidget {
  const ClosingTopBar({super.key});
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    '${loc.role}  ·  ${staff?.employeeId ?? 'LAW-0087'}',
            ...
            IconButton(
              ...
              onPressed: () => context.go('/closing/profile'),
              icon: Badge(
...
List<(IconData, String)> _items(ClosingStrings loc) => [
      (Icons.assignment_outlined, loc.navWork),
      (Icons.account_tree_outlined, loc.navTx),
      (Icons.account_balance_outlined, loc.navFinance),
      (Icons.account_balance, loc.navGov),
      (Icons.folder_outlined, loc.navDocs),
      (Icons.chat_bubble_outline, loc.navMsg),
      (Icons.inventory_2_outlined, loc.navArchive),
      (Icons.person_outline, loc.navProfile),
    ];

class ClosingWorkspaceLoader extends StatefulWidget {
  const ClosingWorkspaceLoader({required this.child, super.key});
  final Widget child;

  @override
  State<ClosingWorkspaceLoader> createState() => _ClosingWorkspaceLoaderState();
}

class _ClosingWorkspaceLoaderState extends State<ClosingWorkspaceLoader> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<ClosingWorkspaceController>().load();
    });
  }
