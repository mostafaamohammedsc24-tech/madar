...
class MappingLoginScreen extends StatefulWidget {
  const MappingLoginScreen({super.key});

  @override
  State<MappingLoginScreen> createState() => _MappingLoginScreenState();
}

class _MappingLoginScreenState extends State<MappingLoginScreen> {
  ...
  Future<void> _submit(MappingAuthNotifier auth) async {
    final ok = await auth.login(employeeId: _id.text, secret: _secret.text);
    if (!mounted) return;
    if (ok) context.go('/mapping/work');
  }
}
