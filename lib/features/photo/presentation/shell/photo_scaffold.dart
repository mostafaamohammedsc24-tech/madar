...
class PhotoScaffold extends StatelessWidget {
  const PhotoScaffold({required this.navigationShell, super.key});
    ...
    return Scaffold(
      ...
      body: rail
          ? Row(children: [
              ...
              Expanded(child: Column(children: [const PhotoTopBar(), Expanded(child: navigationShell)])),
            ])
          : Column(children: [const PhotoTopBar(), Expanded(child: navigationShell), _Bottom(shell: navigationShell)]),
    );
  }
}

class PhotoTopBar extends StatelessWidget {
  const PhotoTopBar({super.key});
  @override
  Widget build(BuildContext context) {
        ...
        child: Row(children: [
          ...
          Expanded(
            child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('${loc.role}  ·  ${staff?.employeeId ?? 'PHO-015'}', style: LegalTheme.ibm(size: 11, color: LegalTheme.muted)),
            ]),
          ),
          if (staff != null)
          ...
          IconButton(tooltip: loc.notifications, onPressed: () => context.go('/photo/profile'), icon: Badge(isLabelVisible: ws.unreadCount > 0, label: Text('${ws.unreadCount}'), child: const Icon(Icons.notifications_none_outlined))),
          IconButton(onPressed: () => context.go('/photo/profile'), icon: const Icon(Icons.person_outline)),
        ]),
      ),
    );
  }
...
class _Bottom extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final items = _items(PhotoStrings.of(AppLocalizations.of(context)));
    return Material(
...
List<(IconData, String)> _items(PhotoStrings loc) => [
      (Icons.camera_alt_outlined, loc.navWork),
      (Icons.assignment_outlined, loc.navAssign),
      (Icons.photo_library_outlined, loc.navMedia),
      (Icons.threed_rotation, loc.navTours),
      (Icons.chat_bubble_outline, loc.navMsg),
      (Icons.inventory_2_outlined, loc.navArchive),
      (Icons.person_outline, loc.navProfile),
    ];

class PhotoWorkspaceLoader extends StatefulWidget {
  const PhotoWorkspaceLoader({required this.child, super.key});
  final Widget child;
  @override
  State<PhotoWorkspaceLoader> createState() => _PhotoWorkspaceLoaderState();
}

class _PhotoWorkspaceLoaderState extends State<PhotoWorkspaceLoader> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) => context.read<PhotoWorkspaceController>().load());
  }
