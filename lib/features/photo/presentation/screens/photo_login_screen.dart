...
class PhotoLoginScreen extends StatefulWidget {
  const PhotoLoginScreen({super.key});
  @override
  State<PhotoLoginScreen> createState() => _PhotoLoginScreenState();
}

class _PhotoLoginScreenState extends State<PhotoLoginScreen> {
  ...
  Future<void> _submit(PhotoAuthNotifier auth) async {
    final ok = await auth.login(employeeId: _id.text, secret: _secret.text);
    if (!mounted) return;
    if (ok) context.go('/photo/work');
  }
}
