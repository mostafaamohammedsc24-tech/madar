...
class LegalLoginScreen extends StatefulWidget {
  const LegalLoginScreen({super.key});

  @override
  State<LegalLoginScreen> createState() => _LegalLoginScreenState();
}

class _LegalLoginScreenState extends State<LegalLoginScreen> {
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Text(loc.notPublicSite, style: LegalTheme.ibm(size: 12, color: LegalTheme.muted)),
            const SizedBox(height: 8),
            Text(loc.roleBoundary, style: LegalTheme.ibm(size: 12, color: LegalTheme.muted)),
            ...
            TextField(
              ...
              decoration: InputDecoration(
                hintText: loc.employeeIdHint,
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(4)),
              ),
              onChanged: (_) => setState(() {}),
            ),
            const SizedBox(height: 16),
            Text(loc.secretCode, style: AuthTypography.caption(context)),
            const SizedBox(height: 8),
            TextField(
              controller: _secret,
              obscureText: _obscure,
              decoration: InputDecoration(
                hintText: loc.secretCode,
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(4)),
            ...
            if (auth.message != null) ...[
              const SizedBox(height: 12),
              AuthErrorBanner(message: loc.loginInvalid),
            ],
            ...
            const SizedBox(height: 12),
            TextButton(
              onPressed: () => context.go('/employee-portal'),
              child: Text(AppLocalizations.of(context).officeBackToUserLogin),
            ),
          ],
        ),
      ),
  ...
  Future<void> _submit(LegalAuthNotifier auth) async {
    final ok = await auth.login(employeeId: _id.text, secret: _secret.text);
    if (!mounted) return;
    if (ok) context.go('/legal/work');
  }
}
